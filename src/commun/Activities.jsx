...
const Activities = () => {
    ...
    const sortActivities = (a, b) => {
        if (filter === "latest") {
            return new Date(b.dateDebut+'T'+b.tempsDebut) - new Date(a.dateDebut+'T'+a.tempsDebut);
        } else {
            return new Date(a.dateDebut+'T'+a.tempsDebut) - new Date(b.dateDebut+'T'+b.tempsDebut);
        }
    };
                ...
                <div className="flex flex-wrap pb-5 items-start justify-start gap-x-4 gap-y-6 overflow-y-scroll">
                    {activities?.sort(sortActivities).map((activity) => (
                        <div key={activity.id} className="w-72 h-96 flex-shrink-0 shadow-gray-400 bg-white shadow-lg rounded-md"  style={{position:"relative"}} >
                            {activity.dateDebut > today && <img className="w-32" src="/Badge/avenir.png" alt="A Venir Bientot"  style={{position:"absolute", top:"-12%", right:"-10%"}}/>}
